...
1. JavaScript alert fired in-browser (proof the vulnerability executed)
2. Rule `100012` fired in Wazuh Discover, with the payload visible in the expanded alert's URL field
...
**Problem: Bruteforce rule fired on every single failure**
Cause: no `frequency`/`timeframe`/`same_source_ip` correlation on initial rule.
Solution: added correlation attributes (Step 2 above).

## Verification Checklist
- [x] SSH bruteforce rule fires only on correlated burst, not single failures
- [x] Bruteforce rule correctly tagged with T1110.001
- [x] XSS rule fires on DVWA reflected XSS payload
- [x] XSS rule confirmed against multiple payload variants
- [x] Both rules visible in Wazuh Discover with correct rule ID and description
